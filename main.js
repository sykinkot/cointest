...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_gsap_TweenMax__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_gsap_TweenMax___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_gsap_TweenMax__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_pxloader_PxLoaderImage__ = __webpack_require__(6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_pxloader_PxLoaderImage___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1_pxloader_PxLoaderImage__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_scrollmagic_scrollmagic_uncompressed_plugins_animation_gsap__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_scrollmagic_scrollmagic_uncompressed_plugins_animation_gsap___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2_scrollmagic_scrollmagic_uncompressed_plugins_animation_gsap__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_proton_js__ = __webpack_require__(8);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_proton_js___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_3_proton_js__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_pxloader__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_pxloader___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_4_pxloader__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5_scrollmagic__ = __webpack_require__(2);
...
const configuration = {
  ...
  scenesList: [
    'redefining-the-crypto-economy',
    'problem',
    'introducing-coinsupply',
    'vision-head',
    'vision-tail',
    'vision-simplicity',
    'vision-transparency',
...
class Particles {
  constructor(config) {
    ...
    this.sceneArea = new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.Rectangle(
      0,
      0,
  ...
  loadMaps() {
    const loader = new __WEBPACK_IMPORTED_MODULE_4_pxloader___default.a();
    const loaderList = this.config.scenesList.map((id) => {
      const container = document.getElementById(id);
      const url = container.getElementsByTagName('img')[0].src;

      return loader.addImage(url);
    });

    loader.addCompletionListener(() => {
      this.zonesList = loaderList.map((item) => {
        const imageData = __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.Util.getImageData(this.context, item, this.sceneArea);

        return new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.ImageZone(imageData, 0, 0);
      });
      this.createProton();
      this.tick();
    });
  ...
  createProton() {
    this.proton = new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a();

    const emitter = new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.Emitter();
    const span = new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.Span(this.config.particles.quantity);

    emitter.rate = new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.Rate(span);
    emitter.addInitialize(new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.Mass(0));
    emitter.addInitialize(new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.Radius(
      this.config.particles.radius.min,
      this.config.particles.radius.max,
    ));
    emitter.addInitialize(new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.P(new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.RectZone(
      this.sceneArea.x,
      this.sceneArea.y,
      this.sceneArea.width,
      this.sceneArea.height,
    )));
    emitter.addBehaviour(this.particleBehaviour());
    emitter.emit('once');
    this.proton.addEmitter(emitter);

    const renderer = new __WEBPACK_IMPORTED_MODULE_3_proton_js___default.a.Renderer('canvas', this.proton, this.canvas);

    renderer.createImageData(this.sceneArea);
    renderer.start();
  }
  ...
  particleBehaviour() {
    return {
      ...
      initialize: (particle) => {
        particle.R = 40 * Math.random();
        particle.Angle = Math.random() * Math.PI * 2;
        particle.speed = Math.random() / 250;
        particle.zones = this.zonesList.map(zone => zone.getPosition().clone());
        particle.colors = this.getColors(particle.zones);
        particle.tha = Math.random() * Math.PI;
        particle.thaSpeed = (0.05 * Math.random()) + 8e-4;
      },
      applyBehaviour: (particle) => {
        particle.v.clear();
        particle.Angle += particle.speed;

        const mapIndex = this.sceneIndex % this.config.scenesList.length;
        const x = particle.zones[mapIndex].x + (particle.R * Math.cos(particle.Angle));
        const y = particle.zones[mapIndex].y + (particle.R * Math.sin(particle.Angle));
        const speed = Math.PI;

        particle.p.x += (x - particle.p.x) * particle.thaSpeed * speed;
        particle.p.y += (y - particle.p.y) * particle.thaSpeed * speed;
        if ([2, 3].includes(mapIndex)) {
          particle.R = Math.random() * {
            2: 10,
            3: 3,
          }[mapIndex];
        }
        particle.transform.rgb.r = particle.colors[0].r;
        particle.transform.rgb.g = particle.colors[0].g;
        particle.transform.rgb.b = particle.colors[0].b;
        particle.tha += 0.001 * particle.thaSpeed;
        particle.alpha = Math.abs(Math.cos(0.5 * particle.tha));
      },
      /* eslint-enable no-param-reassign */
    };
  }

  getColors(positionArray) {
    return positionArray.map((p, index) => (
      this.zonesList[index].getColor(p.x, p.y)
    ));
  }
...
const particles = new Particles(configuration);
const controller = new __WEBPACK_IMPORTED_MODULE_5_scrollmagic___default.a.Controller();

configuration.scenesList.forEach((scene, index) => {
  new __WEBPACK_IMPORTED_MODULE_5_scrollmagic___default.a.Scene({
...
new __WEBPACK_IMPORTED_MODULE_5_scrollmagic___default.a.Scene({
  triggerElement: '#team-and-advisers',
  duration: 300,
})
  .setTween('#particles', { opacity: 0 })
  .addTo(controller);
...
new __WEBPACK_IMPORTED_MODULE_5_scrollmagic___default.a.Scene({
  ...
  .on('enter leave', (e) => {
    ...
    const bars = chart.getElementsByClassName('chart__bar');

    if (e.type === 'enter' && e.scrollDirection === 'FORWARD') {
      chart.classList.remove('chart_zero');
      [].forEach.call(bars, (bar, index) => {
        const height = 100 / (bars.length - 1) * (!(index % 2) ? index : (index - 1) * 2 / 3);

        bar.style.height = `${height}%`;
      });
    }
    if (e.type === 'leave' && e.scrollDirection === 'REVERSE') {
      chart.classList.add('chart_zero');
      [].forEach.call(bars, bar => bar.style.height = 0);
    }
  });
